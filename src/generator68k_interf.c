...
{
    int i;
    debug("d0=%08x   d4=%08x   a0=%08x   a4=%08x   %c%c%c%c%c %04x\n",
    ...
    debug("d1=%08x   d5=%08x   a1=%08x   a5=%08x\n",
	   regs.regs[1],regs.regs[5],regs.regs[9],regs.regs[13]);
    debug("d2=%08x   d6=%08x   a2=%08x   a6=%08x\n",
	   regs.regs[2],regs.regs[6],regs.regs[10],regs.regs[14]);
    debug("d3=%08x   d7=%08x   a3=%08x   a7=%08x   usp=%08x\n",
...
static void hexdump(uint32_t addr) {
    ...
    for(i = 0; i < 8; i++) {
	debug("%08X: %c", tmpaddr,(addr == tmpaddr) ? '>' : ' ' );
	for(j = 0; j < 16; j += 2) {
