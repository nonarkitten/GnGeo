...
void neogeo_bootleg_cx_decrypt( running_machine *machine )
{
	...
	uint8_t *buf = alloc_array_or_die(uint8_t, cx_size );
...
void neogeo_bootleg_sx_decrypt( running_machine *machine, int value )
{
	...
	if (value == 1)
	{
		uint8_t *buf = alloc_array_or_die(uint8_t, sx_size );
...
void kog_px_decrypt( running_machine *machine )
{
	...
	uint8_t *dst = alloc_array_or_die(uint8_t, 0x600000 );
...
{
	int i, j;
	uint8_t *dst = alloc_array_or_die(uint8_t, 0x900000);
	uint8_t *src = memory_region( machine, "maincpu" );
...
void decrypt_kf10thep(running_machine *machine)
{
	...
	uint8_t *sbuf = alloc_array_or_die(uint8_t, 0x20000);

	uint8_t *dst = alloc_array_or_die(uint8_t, 0x200000);
...
static void kf2k5uni_px_decrypt( running_machine *machine )
{
	int i, j, ofst;
	uint8_t *src = memory_region( machine, "maincpu" );
	uint8_t *dst = alloc_array_or_die(uint8_t, 0x80);
...
void kof2002b_gfx_decrypt(running_machine *machine, uint8_t *src, int size)
{
	...
	uint8_t *dst = alloc_array_or_die(uint8_t,  0x10000 );
...
static void cthd2003_neogeo_gfx_address_fix_do(running_machine *machine, int start, int end, int bit3shift, int bit2shift, int bit1shift, int bit0shift)
{
	...
	uint8_t* rom = alloc_array_or_die(uint8_t, 16*tilesize);	// 16 tiles buffer
...
void decrypt_cthd2003( running_machine *machine )
{
	...
	uint8_t *tmp = alloc_array_or_die(uint8_t, 8*128*128);
...
static void ct2k3sp_sx_decrypt( running_machine *machine )
{
	...
	uint8_t *rom = memory_region( machine, "fixed" );
	uint8_t *buf = alloc_array_or_die(uint8_t,  rom_size );
	int i;
	int ofst;
...
void decrypt_kof2k4se_68k( running_machine *machine )
{
	...
	uint8_t *dst = alloc_array_or_die(uint8_t, 0x400000);
	int i;
...
{
	int i;
	uint8_t *rom = memory_region( machine, "ym" );
...
void svcboot_px_decrypt( running_machine *machine )
{
	...
	uint8_t *src = memory_region( machine, "maincpu" );
	uint8_t *dst = alloc_array_or_die(uint8_t,  size );
	int ofst;
...
static WRITE16_HANDLER( kof2003_w )
{
	...
	if (offset == 0x1ff0/2 || offset == 0x1ff2/2) {
		...
		uint32_t address = (cr[uint8_t_XOR_LE(0x1ff3)]<<16)|(cr[uint8_t_XOR_LE(0x1ff2)]<<8)|cr[uint8_t_XOR_LE(0x1ff1)];
		uint8_t prt = cr[uint8_t_XOR_LE(0x1ff2)];
		uint8_t* mem = (uint8_t *)memory_region(space->machine, "maincpu");

		cr[uint8_t_XOR_LE(0x1ff0)] =  0xa0;
		cr[uint8_t_XOR_LE(0x1ff1)] &= 0xfe;
		cr[uint8_t_XOR_LE(0x1ff3)] &= 0x7f;
		neogeo_set_main_cpu_bank_address(space, address+0x100000);

		mem[uint8_t_XOR_LE(0x58196)] = prt;
	}
}
...
static WRITE16_HANDLER( kof2003p_w )
{
	...
	if (offset == 0x1ff0/2 || offset == 0x1ff2/2) {
		uint8_t* cr = (uint8_t *)kof2003_tbl;
		uint32_t address = (cr[uint8_t_XOR_LE(0x1ff3)]<<16)|(cr[uint8_t_XOR_LE(0x1ff2)]<<8)|cr[uint8_t_XOR_LE(0x1ff0)];
		uint8_t prt = cr[uint8_t_XOR_LE(0x1ff2)];
		uint8_t* mem = (uint8_t *)memory_region(space->machine, "maincpu");

		cr[uint8_t_XOR_LE(0x1ff0)] &= 0xfe;
		cr[uint8_t_XOR_LE(0x1ff3)] &= 0x7f;
		neogeo_set_main_cpu_bank_address(space, address+0x100000);
...
{
	uint16_t*tmp = alloc_array_or_die(uint16_t, 0x100000/2);
	uint16_t*rom = (uint16_t*)memory_region( machine, "maincpu" );
	int j;
	int i;
...
void samsho5b_px_decrypt( running_machine *machine )
{
	...
	uint8_t *rom = memory_region( machine, "maincpu" );
	uint8_t *buf = alloc_array_or_die(uint8_t,  px_size );
	int ofst;
	int i;
...
void matrimbl_decrypt( running_machine *machine )
{
	...
	uint8_t *buf = alloc_array_or_die(uint8_t,  0x20000 );
	int i, j=0;
