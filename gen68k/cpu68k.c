...
void cpu68k_ipc(uint32_t addr68k, uint8_t *addr, t_iib * iib, t_ipc * ipc)
{
  ...
  if ((iib->mnemonic == i_Bcc) || (iib->mnemonic == i_BSR)) {
    ...
    ipc->src = (int32_t)(*(int8_t *)(addr + 1));
    if (ipc->src == 0) {
      ipc->src = (int32_t)(int16_t)LOCENDIAN16(*(uint16_t *)(addr + 2));
      ipc->wordlen++;
    }
  ...
  for (type = 0; type < 2; type++) {
    ...
    case dt_Adis:
      *p = (int32_t)(int16_t)LOCENDIAN16(*(uint16_t *)addr);
      ipc->wordlen++;
      addr += 2;
      addr68k += 2;
      break;
    case dt_Aidx:
      *p = (int32_t)(int8_t)addr[1];
    ...
    case dt_Pdis:
      ...
      *p += addr68k;            /* add PC of extension word (this word) */
      ...
      break;
    case dt_Pidx:
      *p = ((int32_t)(int8_t)addr[1]) + addr68k;
      *p = (*p & 0xFFFFFF) | (*addr) << 24;
      ipc->wordlen++;
      addr += 2;
      addr68k += 2;
    ...
    case dt_Imm8s:
      if (type == tp_src)
        *p = (int32_t)(int8_t)((ipc->opcode >> iib->sbitpos) & 255);
      else
        *p = (int32_t)(int8_t)((ipc->opcode >> iib->dbitpos) & 255);
      break;
    default:
      break;
    }
...
t_ipclist *cpu68k_makeipclist(uint32_t pc)
{
  ...
  do {
    ...
    if (instrs > size) {
	if (size > 10000) {
	    debug("Something has gone seriously wrong @ %08X", (unsigned)pc);
	    exit(1);
	}
	size += 16;
	list = realloc(list, sizeof(t_ipclist) + size * sizeof(t_ipc) + 8);
	if (list == NULL) {
	    debug("Out of memory whilst making ipc list @ %08X",
		(unsigned)pc);
	    exit(1);
	}
	...
	debug("Invalid instruction @ %08X [%04X]", (unsigned)pc,
	    fetchword(pc));
	exit(1);
    }
