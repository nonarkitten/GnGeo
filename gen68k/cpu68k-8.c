...
void cpu_op_1119a(t_ipc *ipc) /* DIVU */ {
  ...
  if ((quotient & 0xffff0000) == 0) {
    DATAREG(dstreg) = quotient | (((uint16_t)(dstdata % srcdata))<<16);
  }
  PC+= 2;
}
...
void cpu_op_1119b(t_ipc *ipc) /* DIVU */ {
  ...
  if ((quotient & 0xffff0000) == 0) {
    DATAREG(dstreg) = quotient | (((uint16_t)(dstdata % srcdata))<<16);
    VFLAG = 0;
    NFLAG = ((int16_t)quotient) < 0;
  ZFLAG = !((uint16_t)quotient);
  } else {
    VFLAG = 1;
    NFLAG = 1;
  }
...
void cpu_op_1130a(t_ipc *ipc) /* DIVS */ {
  ...
  uint16_t srcdata = DATAREG(srcreg);
  int dstreg = (ipc->opcode >> 9) & 7;
  int32_t dstdata = DATAREG(dstreg);
  int32_t quotient;
  int16_t remainder;
  ...
  quotient = dstdata / (int16_t)srcdata;
  remainder = dstdata % (int16_t)srcdata;
  if (((quotient & 0xffff8000) == 0) ||
      ((quotient & 0xffff8000) == 0xffff8000)) {
    if (((int32_t)dstdata < 0) != (remainder < 0))
      remainder = -remainder;
    DATAREG(dstreg) = ((uint16_t)quotient) | (((uint16_t)(remainder))<<16);
  }
  PC+= 2;
}
...
void cpu_op_1141a(t_ipc *ipc) /* SBCD */ {
  ...
  int8_t outdata_low = (dstdata & 0xF) - (srcdata & 0xF) - XFLAG;
  int16_t precalc = dstdata - srcdata - XFLAG;
