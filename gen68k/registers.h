...
#if 0
#ifdef PROCESSOR_ARM
     register uint32_t reg68k_pc asm ("r7");
     register uint32_t *reg68k_regs asm ("r8");
     register t_sr reg68k_sr asm ("r9");
#else
/* This doesn't seem to work.... (freeze the emulator)

#  ifdef PROCESSOR_SPARC
       register uint32_t reg68k_pc asm ("5");
       register uint32_t *reg68k_regs asm ("6");
       register t_sr reg68k_sr asm ("7");
#  else
*/
#    ifdef PROCESSOR_INTEL
         register uint32_t reg68k_pc asm ("%ebx");
         register uint32_t *reg68k_regs asm ("%edi");
         register t_sr reg68k_sr asm ("%esi");
#    else

         extern uint32_t reg68k_pc;
         extern uint32_t *reg68k_regs;
